    ...
    useEffect(() => {
        const fetchWallets = async () => {
            try {
                ...
                const response = await fetch(`http://localhost:3000/api/investment/${id}`, {
                    method: "GET",
                    headers: {
                ...
                console.log(responseDate.investments);
                investments.add(responseDate.investments);
                setInvestments(investments);
                investments.getCategories();
                console.log(investments.categories.get("FIIS"));
            } catch (e) {
        ...
        <div className="d-flex flex-column align-items-center bg-dark vh-100">
            ...
            >
                ...
                >
                    ...
                    {investments.getCategories().map((category: any) => (
                        <div
                            key={category.name}
...
class Investment {
    ...
    public add(investments: any): void {
    ...
    public getCategories(): any {
        this.categories = new Map<string, { list: any[]; currency: string }>();

        for (const investment of this.investments) {
            if (!this.categories.has(investment.category)) 
                this.categories.set(investment.category, { list: [], currency: investment.currency });
            this.categories.get(investment.category)!.list.push(investment);
        }
        const summaries: any[] = [];
        for (const [name, { list, currency }] of this.categories) {
            const { value, amount } = list.reduce(
                (acc: any, inv: any) => ({
                    value: acc.value + (inv.currentValue * inv.quantity),
                    amount: acc.amount + (inv.average * inv.quantity),
                }),
                { value: 0, amount: 0 }
            );

            summaries.push({ name, value, amount, currency });
        }
